...
const formatSingleDateTime = (dateString: string | null | undefined): string => {
    if (!dateString) return '';
    try {
        const date = new Date(dateString);
        if (isNaN(date.getTime())) {
            console.warn(`Invalid date string passed to formatSingleDateTime: ${dateString}`);
                    ...
                    <div key={event.id} className="event-card">
                        <div>
                            ...
                            <div className="event-card-meta">
                                <span>
                                    <CalendarIcon /> {formatSingleDateTime(event.start_datetime)}
                                </span>
                                {event.end_datetime && (
                                    <span title="Время окончания">
                                        <span style={{ paddingLeft: '22px' }}>
                                             до: {formatSingleDateTime(event.end_datetime)}
                                        </span>
                                    </span>
                                )}
